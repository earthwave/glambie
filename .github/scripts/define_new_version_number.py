...
def main(version_txt_filename: str, major_minor_version_txt_filename: str) -> None:
    ...
    try:
        version_tags = (
            subprocess.check_output("git tag --sort=authordate | grep v", shell=True)
            .decode()
            .split("\n")[:-1]
        )
        ...
        print(
            log_prefix
            + "Did not detect any tags previously existing in this repository."
        )
        version_tags = []
    ...
    with open(major_minor_version_txt_filename, "r") as file:
        ...
        print(
            log_prefix
            + f"Major/minor version number read from file {major_minor_version_txt_filename} "
    ...
    for tag in reversed(version_tags):
        if (
            major_minor_version in tag
            and not found_previous_build_of_this_major_minor_version
        ):
            print(
                log_prefix
                + f"Found tag {tag} which appears to indicate last matching build."
            )
            found_previous_build_of_this_major_minor_version = True
            current_build_number = re.findall(r"\d+", tag)[-1]
            new_version_number = tag[: tag.rfind(current_build_number)] + str(
                int(current_build_number) + 1
            )
    ...
    if not found_previous_build_of_this_major_minor_version:
        ...
        print(
            log_prefix
            + f"Did not find any tags matching major/minor version {major_minor_version}, "
    ...
    with open(version_txt_filename, "w") as file:
        ...
        print(
            log_prefix
            + f"Wrote new version number {new_version_number} to {version_txt_filename}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--version_txt_filename",
        type=str,
        default="full_version.txt",
        help="This script will create a file with this name containing the next verison number.",
    )

    parser.add_argument(
        "--major_minor_version_txt_filename",
        type=str,
        default="major_minor_version.txt",
    ...
    main(
        version_txt_filename=parsed_args.version_txt_filename,
