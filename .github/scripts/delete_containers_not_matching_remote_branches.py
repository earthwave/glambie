...
def main(package_name: str, dry_run: bool) -> None:
    ...
    if package_name is None:
        ...
        print(
            log_prefix
            + f"no package name specified, assuming package name is {package_name}"
        )

    # first fetch a list of all docker images (as well as other information) from the repository
    all_docker_images = (
        subprocess.check_output(
            "gcloud artifacts docker images list europe-west1-docker.pkg.dev/glambie/dr",
            shell=True,
        )
        .decode()
        .split("\n")[1:-1]
    )  # strip off header and trailing newline with indexing
    ...
    docker_images_for_this_package = list(
        {
            image_str.split()[0]
            for image_str in all_docker_images
    ...
    all_branch_refs = (
        subprocess.check_output("git fetch --prune && git branch -r", shell=True)
        .decode()
        .split("\n")
    )
    ...
    branch_names_for_this_package = [
        branch_ref.split("/")[1]
        for branch_ref in all_branch_refs
    ...
    for image_name in docker_images_for_this_package:
        if not any(
            image_name.endswith(branch_name)
            for branch_name in branch_names_for_this_package
        ):
            log_message = (
                log_prefix + f"Deleted image {image_name} from the Artifact repo"
            )
            ...
            if dry_run:
                ...
                subprocess.run(
                    f"gcloud artifacts docker images delete {image_name} --delete-tags --quiet",
                    shell=True,
                )
                print(log_message)
