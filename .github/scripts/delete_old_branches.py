...
def main(branch_age_limit_days: int, dry_run: bool) -> None:
    ...
    for branch_name in branch_names:
        posix_date_of_youngest_commit = int(
        ...
        days_since_last_commit = int(
            (datetime.utcnow().timestamp() - posix_date_of_youngest_commit) // 86400
        )
        print(
            log_prefix
            + f"The last commit on remote branch {branch_name} was made {days_since_last_commit} "
        ...
        if days_since_last_commit > branch_age_limit_days:
            ...
            log_message = (
                log_prefix
                + f"Deleted branch {branch_name} because the last commit was "
            ...
            if dry_run:
                ...
                subprocess.check_output(
                    f"git push origin --delete {branch_name}", shell=True
                )
                print(log_message)
...
if __name__ == "__main__":  # pragma: no cover
    ...
    main(
        branch_age_limit_days=parsed_args.branch_age_limit_days,
