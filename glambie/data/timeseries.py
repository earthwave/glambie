...
class TimeseriesData:
    ...
    def min_temporal_resolution(self) -> float:
        ...
        resolution = np.max(
            [
                ...
                for end, start in zip(self.end_dates, self.start_dates)
            ]
        )
    ...
    def as_cumulative_timeseries(self) -> pd.DataFrame:
        ...
        if not self.is_cumulative_valid():
            log.debug(
                "Cumulative timeseries may be invalid. This may be due to the timeseries containing "
                "gaps or overlapping periods."
            )
        dates, changes = derivative_to_cumulative(
            self.start_dates, self.end_dates, self.changes, add_gaps_for_plotting=True
        )
        _, errors = derivative_to_cumulative(
            self.start_dates,
            self.end_dates,
            self.errors,
        ...
        df_cumulative = pd.DataFrame(
            {"dates": dates, "changes": changes, "errors": errors}
        )
    ...
    def is_cumulative_valid(self):
        ...
        return all(
            start_date == end_date
            for start_date, end_date in zip(self.start_dates[1:], self.end_dates[:-1])
        )
...
class Timeseries:
    ...
    def __init__(
        self,
        region: RGIRegion = None,
        data_group: GlambieDataGroup = None,
    ...
    def timeseries_is_monthly_grid(self):
        ...
        """
        return timeseries_is_monthly_grid(
            self.data.start_dates
        ) and timeseries_is_monthly_grid(self.data.end_dates)

    def timeseries_is_annual_grid(
        self, year_type: constants.YearType = constants.YearType.CALENDAR
    ):
        """
        Returns True if all values in the self.data.start_dates and self.data.end_dates are on
        ...
        return all(s % 1 == year_start for s in self.data.start_dates) and all(
            s % 1 == year_start for s in self.data.end_dates
        )
    ...
    def convert_timeseries_to_unit_mwe(
        ...
        if self.unit == "mwe":  # no conversion needed as already in mwe
            ...
            if str.lower(self.unit) == "m":
                object_copy.data.changes = np.array(
                ...
                object_copy.data.errors = np.array(
                    meters_to_meters_water_equivalent(
                        object_copy.data.errors,
                        density_of_water=density_of_water,
                        density_of_ice=density_of_ice,
                    )
                )
                # Second, include density uncertainty in error
                density_unc = get_density_uncertainty_over_survey_period(
                    self.data.max_temporal_resolution
                )
                ...
                errors_mwe = (
                    df.changes.abs()
                    * (
                        (df.errors / df.changes) ** 2
                        + (density_unc / density_of_ice) ** 2
                    )
                    ** 0.5
                )
                ...
                # get area
                glacier_area = getattr(
                    self.region, "rgi{}_area".format(str(rgi_area_version))
                )
                ...
                errors_area_unc_removed = (
                    np.abs(  # inverting the formula from glambie ATBD
                        object_copy.data.changes
                        * np.abs(
                            object_copy.data.errors**2 * glacier_area**2
                            - np.abs(object_copy.data.changes) ** 2 * area_unc**2
                        )
                        ** 0.5
                    )
                ...
                object_copy.data.errors = np.array(
                    gigatonnes_to_meters_water_equivalent(
                        errors_area_unc_removed,
                        glacier_area,
                        density_of_water=density_of_water,
                    )
                ...
                raise NotImplementedError(
                    "Conversion to mwe not implemented yet for Timeseries with unit '{}'".format(
                        self.unit
                    )
                )

    def convert_timeseries_to_unit_gt(
        self,
        rgi_area_version: int,
        density_of_water: float = constants.DENSITY_OF_WATER_KG_PER_M3,
    ) -> Timeseries:
        ...
        if str.lower(self.unit) == "gt":  # no conversion needed as already in gt
            ...
            # area_unc is calculated as a % of the total area. % can be defined individually per region.
            area_unc = (
                glacier_area * self.region.area_uncertainty_percentage
            )  # use individual glacier area unc
            area = glacier_area
            ...
            uncertainties_gt = (
                df.changes.abs()
                * ((df.errors / df.changes) ** 2 + (area_unc / area) ** 2) ** 0.5
            )
            object_copy.data.errors = np.array(uncertainties_gt)
            return object_copy
        else:
            raise NotImplementedError(
                "Conversion to Gt not implemented yet for Timeseries with unit '{}'".format(
                    self.unit
                )
            )

    def apply_or_remove_area_change(
        self, rgi_area_version: int, apply_area_change: bool = True
    ) -> Timeseries:
        """
        ...
        for start_date, end_date, change in zip(
            df["start_dates"], df["end_dates"], df["changes"]
        ):
            adjusted_area = self.region.get_adjusted_area(
        ...
        object_copy.area_change_applied = (
            apply_area_change  # store if has been applied or not
        )
        return object_copy

    def reduce_to_date_window(
        self, start_date: float, end_date: float, date_window_is_gap: bool = False
    ) -> Timeseries:
        """
        ...
        if date_window_is_gap:  # remove values between start and end date
            df = df[
                ~((df.end_dates <= end_date) & (df.start_dates >= start_date))
            ].reset_index(drop=True)
        else:  # remove values smaller than start date or larger than end date
            df = df[
                ((df.end_dates <= end_date) & (df.start_dates >= start_date))
            ].reset_index(drop=True)
        object_copy.data = TimeseriesData(
    ...
    def convert_timeseries_to_monthly_grid(self) -> Timeseries:
        ...
        object_copy = self.copy()
        if (
            not self.timeseries_is_monthly_grid()
            ...
            if (
                ...
                end_dates = timeseries_as_months(
                    self.data.end_dates, downsample_to_month=False
                )
                ...
                _, _, errors = resample_derivative_timeseries_to_monthly_grid(
                    self.data.start_dates, self.data.end_dates, self.data.errors
                )
    ...
    def convert_timeseries_to_annual_trends(
        ...
        if self.data.max_temporal_resolution <= 1:  # resolution higher than a year
            min_date, max_date = (
                np.array(self.data.start_dates).min(),
                np.array(self.data.end_dates).max(),
            )
            new_start_dates, new_end_dates = get_years(
                year_start, min_date=min_date, max_date=max_date, return_type="arrays"
            )
            df_annual = get_average_trends_over_new_time_periods(
                start_dates=self.data.start_dates,
                end_dates=self.data.end_dates,
                changes=self.data.changes,
                new_start_dates=new_start_dates,
                new_end_dates=new_end_dates,
            )
            df_annual_errors = get_average_trends_over_new_time_periods(
                start_dates=self.data.start_dates,
                end_dates=self.data.end_dates,
    ...
    def convert_timeseries_to_longterm_trend(
        self,
    ...
    def shift_timeseries_to_annual_grid_with_seasonal_homogenization(
        ...
        object_copy = self.copy()
        if not self.timeseries_is_annual_grid(
            year_type=year_type
        ):  # if already annual then no need to homogenize
            ...
            df_mean_calibrated = pd.DataFrame(
                {
                    "start_dates": seasonal_calibration_dataset.data.as_dataframe().start_dates,
                    "end_dates": seasonal_calibration_dataset.data.as_dataframe().end_dates,
                    "changes": mean_calibrated_ts,
                }
            )
            # 3) remove nan values where timeseries didn't cover
            df_mean_calibrated = df_mean_calibrated[
                ~df_mean_calibrated.isna()
            ].reset_index()
            ...
            annual_grid = get_years(
                year_start,
                min_date=self.data.start_dates.min() - 2,
                max_date=self.data.end_dates.max() + 3,
                return_type="arrays",
            )[0]
            new_start_dates, new_end_dates, new_changes, new_errors = [], [], [], []
            for start_date, end_date, error in zip(
                self.data.start_dates, self.data.end_dates, self.data.errors
            ):
                ...
                df_filtered_year = df_mean_calibrated[
                    (df_mean_calibrated.start_dates >= new_start_date)
                ...
                if df_filtered_year.shape[0] == 0:
                    ...
                    if (
                        df_filtered_year.start_dates.min() == new_start_date
                        ...
                        df_mean_calibrated_cumulative.changes = (
                            df_mean_calibrated.changes.cumsum()
                        )
                        df_filtered_year_cum_initial_dates = (
                            df_mean_calibrated_cumulative[
                                (
                                    df_mean_calibrated_cumulative.start_dates
                                    >= start_date
                                )
            ...
            # remove nan values
            df_nan_removed = object_copy.data.as_dataframe()[
                ~object_copy.data.as_dataframe()["changes"].isna()
            ]
    ...
    def shift_timeseries_to_annual_grid_proportionally(
        ...
        if not self.timeseries_is_annual_grid(
            ...
            new_start_dates, new_end_dates, new_changes, new_errors = [], [], [], []
            for start_date, end_date, change, error in zip(
                self.data.start_dates,
                self.data.end_dates,
                self.data.changes,
                self.data.errors,
                ...
                new_changes.append(
                    float(change / period_length_old * period_length_new)
                )
