...
from glambie.processing.processing_helpers import (
    convert_datasets_to_monthly_grid,
    get_reduced_catalogue_to_date_window,
)
...
from glambie.plot.plot_helpers import (
    ...
    finalise_save_to_file_and_close_plot,
)
from glambie.plot.plot_helpers import (
...
def plot_all_plots_for_region_data_group_processing(
    ...
    plot_fp = output_path_handler.get_plot_output_file_path(
        region=region, data_group=data_group, plot_file_name="1_annual_input_data.png"
    )
    ...
    if not data_catalogue_annual_raw.datasets_are_same_unit():  # annual
        data_catalogue_annual_raw = convert_datasets_to_unit_mwe(
            data_catalogue_annual_raw, rgi_area_version=rgi_area_version
        )
    if not data_catalogue_trends_raw.datasets_are_same_unit():  # trends
        data_catalogue_trends_raw = convert_datasets_to_unit_mwe(
            data_catalogue_trends_raw, rgi_area_version=rgi_area_version
        )
    ...
    data_catalogue_annual_homogenized = get_reduced_catalogue_to_date_window(
        data_catalogue=data_catalogue_annual_homogenized,
        start_date=min_date,
        end_date=max_date,
    )
    data_catalogue_annual_anomalies = get_reduced_catalogue_to_date_window(
        data_catalogue=data_catalogue_annual_anomalies,
        start_date=min_date,
        end_date=max_date,
    )
    ...
    plot_raw_input_data_of_data_group(
        catalogue_raw=data_catalogue_annual_raw,
    ...
    plot_raw_and_homogenized_input_data_of_data_group(
        catalogue_raw=data_catalogue_annual_raw,
    ...
    plot_homogenized_input_data_of_data_group(
        catalogue_annual=data_catalogue_annual_homogenized,
        trend_combined=timeseries_trend_combined,
        data_group=data_group,
        region=region,
        output_filepath=plot_fp,
    ...
    plot_fp = output_path_handler.get_plot_output_file_path(
        region=region, data_group=data_group, plot_file_name="4_annual_variability.png"
    )
    plot_annual_variability_of_data_group(
        catalogue_annual_anomalies=data_catalogue_annual_anomalies,
    ...
    plot_fp = output_path_handler.get_plot_output_file_path(
        region=region, data_group=data_group, plot_file_name="5_trends_input_data.png"
    )
    plot_raw_input_data_of_data_group(
        catalogue_raw=data_catalogue_trends_raw,
        trend_combined=timeseries_trend_combined,
        data_group=data_group,
        region=region,
        category="trends",
    ...
    data_catalogue_trends_raw = convert_datasets_to_monthly_grid(
        data_catalogue_trends_raw
    )
    data_catalogue_trends_raw = convert_datasets_to_unit_mwe(
        data_catalogue_trends_raw, rgi_area_version=rgi_area_version
    )
    ...
    plot_recalibration_of_annual_variability_with_trends(
        catalogue_trends=data_catalogue_trends_homogenized,
    ...
    plot_fp = output_path_handler.get_plot_output_file_path(
        region=region, data_group=data_group, plot_file_name="8_combined_result.png"
    )
    plot_recalibrated_result_of_data_group(
        catalogue_trends=data_catalogue_trends_homogenized,
        catalogue_calibrated_series=data_catalogue_calibrated_series,
        trend_combined=timeseries_trend_combined,
        data_group=data_group,
...
def plot_raw_input_data_of_data_group(
    catalogue_raw: DataCatalogue,
    ...
    add_labels_axlines_and_title(
        ...
        unit=unit,
        legend_fontsize=7,
        title="{} - {} - {}: raw input data".format(
            region.long_name, data_group.long_name, category
        ),
    )
    finalise_save_to_file_and_close_plot(output_filepath)


def plot_raw_and_homogenized_input_data_of_data_group(
    catalogue_raw: DataCatalogue,
    ...
    colours = get_colours(
        max([len(catalogue_raw.datasets), len(catalogue_homogenized.datasets)])
    )
...
def plot_homogenized_input_data_of_data_group(
    catalogue_annual: DataCatalogue,
    trend_combined: DataCatalogue,
    data_group: GlambieDataGroup,
    region: RGIRegion,
    output_filepath: str,
    ...
    add_labels_axlines_and_title(
        axes=axes,
        unit=catalogue_annual.datasets[0].unit,
        legend_fontsize=7,
        title="{} - {}: homogenized input data".format(
            region.long_name, data_group.long_name
        ),
    )
    finalise_save_to_file_and_close_plot(output_filepath)


def plot_annual_variability_of_data_group(
    catalogue_annual_anomalies: DataCatalogue,
    ...
    plot_non_cumulative_timeseries_on_axis(
        result_dataframe=timeseries_combined_annual.data.as_dataframe(),
        ax=axes[0],
        colour="black",
        linestyle="--",
        plot_errors=plot_errors,
    ...
    plot_cumulative_timeseries_on_axis(
        timeseries=timeseries_combined_annual,
        ax=axes[1],
        colour="black",
        plot_errors=plot_errors,
        linestyle="--",
    ...
    add_labels_axlines_and_title(
        axes=axes,
        unit=catalogue_annual_anomalies.datasets[0].unit,
        legend_fontsize=7,
        title="{} - {}: annual anomalies".format(
            region.long_name, data_group.long_name
        ),
    )
    finalise_save_to_file_and_close_plot(output_filepath)
...
def plot_recalibration_of_annual_variability_with_trends(
    catalogue_trends: DataCatalogue,
    ...
    # plot non-cumulative timeseries
    for count, (ds_trend, ds_annual) in enumerate(
        zip(catalogue_trends.datasets, catalogue_calibrated_series.datasets)
    ):
        plot_non_cumulative_timeseries_on_axis(
            result_dataframe=ds_trend.data.as_dataframe(),
            ax=axes[0],
            colour=colours[count],
        ...
        plot_non_cumulative_timeseries_on_axis(
            result_dataframe=ds_annual.data.as_dataframe(),
            ax=axes[0],
            colour=colours[count],
            linestyle="--",
    ...
    # plot cumulative
    for count, (trends, annual) in enumerate(
        zip(catalogue_trends.datasets, catalogue_calibrated_series.datasets)
    ):
        plot_cumulative_timeseries_on_axis(
            timeseries=trends,
    ...
    add_labels_axlines_and_title(
        axes=axes,
        unit=catalogue_trends.datasets[0].unit,
        title="{} - {}: recalibration with trends".format(
            region.long_name, data_group.long_name
        ),
        legend_fontsize=7,
    )
...
def plot_recalibrated_result_of_data_group(
    catalogue_trends: DataCatalogue,
    catalogue_calibrated_series: DataCatalogue,
    ...
    for count, ds in enumerate(catalogue_calibrated_series.datasets):
        plot_non_cumulative_timeseries_on_axis(
            ...
            colour=colours[count],
            linestyle="--",
            label="Calibrated annual series: "
            + ds.user_group
            + " ("
            + ds.data_group.name
            + ")",
    ...
    plot_non_cumulative_timeseries_on_axis(
        result_dataframe=trend_combined.data.as_dataframe(),
        ax=axes[0],
        colour="black",
        linestyle="--",
    ...
    add_labels_axlines_and_title(
        ...
        unit=catalogue_trends.datasets[0].unit,
        legend_fontsize=7,
        title="{} - {}: combined solution".format(
            region.long_name, data_group.long_name
        ),
    )
    finalise_save_to_file_and_close_plot(output_filepath)
...
def plot_combination_of_sources_within_region(
    catalogue_results: DataCatalogue,
    ...
    for count, df in enumerate(catalogue_results.datasets):
        plot_non_cumulative_timeseries_on_axis(
            result_dataframe=df.data.as_dataframe(),
            ax=axes[0],
            colour=colours[count],
    ...
    plot_non_cumulative_timeseries_on_axis(
        result_dataframe=combined_timeseries.data.as_dataframe(),
        ax=axes[0],
        colour="black",
        linestyle="--",
    ...
    add_labels_axlines_and_title(
        axes=axes,
        unit=combined_timeseries.unit,
        title="{}: combined estimate".format(region.long_name),
        legend_fontsize=7,
    )
    finalise_save_to_file_and_close_plot(output_filepath)


def plot_combination_of_regions_to_global(
    catalogue_region_results: DataCatalogue,
    ...
    for count, timeseries in enumerate(catalogue_region_results.datasets):
        plot_non_cumulative_timeseries_on_axis(
            result_dataframe=timeseries.data.as_dataframe(),
            ax=axes[0],
            colour=colours[count],
    ...
    if global_timeseries is not None:
        plot_non_cumulative_timeseries_on_axis(
            result_dataframe=global_timeseries.data.as_dataframe(),
            ax=axes[0],
            colour="black",
            linestyle="--",
