...
from glambie.data.data_catalogue_helpers import (
...
from glambie.processing.output_helpers import (
...
from glambie.processing.processing_helpers import (
    check_and_handle_gaps_in_timeseries,
    convert_datasets_to_annual_trends,
    convert_datasets_to_longterm_trends_in_unit_mwe,
...
def run_one_region(
    ...
    for data_group in glambie_run_config.datagroups_to_calculate:
        ...
        data_catalogue_annual, data_catalogue_trends = (
            filter_catalogue_with_config_settings(
        ...
        if (
            not len(data_catalogue_annual.datasets)
            ...
            # apply area change
            trend_combined = trend_combined.apply_or_remove_area_change(
                glambie_run_config.rgi_area_version, apply_area_change=True
            )
            # save out with area change applied
            if output_path_handler is not None:
                trend_combined.save_data_as_csv(
                    output_path_handler.get_csv_output_file_path(
                        region=trend_combined.region,
                        data_group=data_group,
            ...
            log.info(
                "Skipping to process region=%s datagroup=%s as no data found.",
                region_config.region_name,
                data_group.name,
            )

    result_catalogue = DataCatalogue.from_list(
        result_datasets, base_path=data_catalogue.base_path
    )
...
def _prepare_consensus_variability_for_one_region(
    ...
    log.info(
        "Starting to process consensus variability for region=%s",
        region_config.region_name,
    )
    ...
    consensus_annual = combine_within_one_region(
        result_catalogue, output_path_handler=None
    )

    # now deal with cases where the consensus does not cover all of the desired period
    # we will fall back on a backup dataset
    backup_dataset = backup_dataset.convert_timeseries_to_annual_trends(
        year_type=year_type
    )
    ...
    consensus_annual_full_ext.data.errors = np.zeros(
        len(consensus_annual_full_ext.data.errors)
    )
...
def combine_within_one_region(
    catalogue_data_group_results: DataCatalogue, output_path_handler: OutputPathHandler
) -> Timeseries:
    """
    ...
    """
    # combine
    combined_ts, _ = catalogue_data_group_results.average_timeseries_in_catalogue(
        remove_trend=True,
        add_trend_after_averaging=True,
        out_data_group=GLAMBIE_DATA_GROUPS["consensus"],
    )
    if output_path_handler is not None:
        output_path = output_path_handler.get_plot_output_file_path(
            region=combined_ts.region,
        ...
        for output_filepath, plot_errors in (
            (output_path, False),
            (output_path_unc, True),
        ):
            plot_combination_of_sources_within_region(
                catalogue_results=catalogue_data_group_results,
                combined_timeseries=combined_ts,
                region=combined_ts.region,
...
def convert_and_save_one_region_to_gigatonnes(
    ...
    combined_region_timeseries_gt = (
        combined_region_timeseries.apply_or_remove_area_change(
            rgi_area_version=rgi_area_version, apply_area_change=False
        )
    )
    combined_region_timeseries_gt = (
        combined_region_timeseries_gt.convert_timeseries_to_unit_gt(
            rgi_area_version=rgi_area_version
        )
    )
    catalogue_data_group_results_gt = convert_datasets_to_unit_gt(
        catalogue_data_group_results, rgi_area_version=rgi_area_version
    )
    ...
    if output_path_handler is not None:
        ...
        for output_filepath, plot_errors in (
            ...
            plot_combination_of_sources_within_region(
                catalogue_results=catalogue_data_group_results_gt,
        ...
        # save csv
        combined_region_timeseries_gt.save_data_as_csv(
            output_path_handler.get_csv_output_file_path(
                region=combined_region_timeseries_gt.region,
                data_group=GLAMBIE_DATA_GROUPS["consensus"],
...
def _run_region_timeseries_for_one_source(
    ...
    data_catalogue_trends, split_dataset_names_trends = (
        check_and_handle_gaps_in_timeseries(data_catalogue_trends)
    )
    ...
    # 1) ANNUAL TRENDS
    log.info(
        "Calculating combined annual trends within data group %s and region %s",
    ...
    if len(data_catalogue_annual.datasets) == 0:
        log.info(
            "No annual dataset found for %s in region %s, using backup dataset %s",
            data_group.long_name,
            region.long_name,
            annual_backup_dataset.user_group,
        )
        data_catalogue_annual = DataCatalogue.from_list([annual_backup_dataset])

    log.info(
        "Using the following annual datasets for %s, %s : %s",
        data_group.long_name,
        region.long_name,
        str([d.user_group for d in data_catalogue_annual.datasets]),
    )
    ...
    # 2) LONGTERM TRENDS
    log.info(
        "Using the following trend datasets for %s, %s : %s",
        data_group.long_name,
        region.long_name,
        str([d.user_group for d in data_catalogue_trends.datasets]),
    )

    trend_combined, data_catalogue_trends_homogenized, catalogue_calibrated_series = (
        _run_region_trends_for_one_source(
            data_catalogue_trends=data_catalogue_trends,
            seasonal_calibration_dataset=seasonal_calibration_dataset,
            annual_backup_dataset=annual_backup_dataset,
            annual_combined_dataset=annual_combined,
            year_type=year_type,
    ...
    if output_path_handler is not None:
        log.info(
            "Saving plots for region=%s datagroup=%s under path=%s",
            region.name,
            data_group.name,
            output_path_handler.get_plot_output_file_path(
                region=region, data_group=data_group, plot_file_name=""
            ),
        )
...
def _run_region_variability_for_one_source(
    ...
    data_catalogue_annual_homogenized = convert_datasets_to_unit_mwe(
        data_catalogue_annual_homogenized, rgi_area_version=rgi_area_version
    )
    ...
    annual_combined, catalogue_annual_anomalies = (
        data_catalogue_annual_homogenized.average_timeseries_in_catalogue(
            remove_trend=True, out_data_group=data_group
        )
    )
    return (
        annual_combined,
        data_catalogue_annual_homogenized,
        catalogue_annual_anomalies,
    )
...
def _run_region_trends_for_one_source(
    ...
    annual_combined_full_ext = extend_annual_timeseries_if_shorter_than_time_window(
        ...
        timeseries_for_extension=annual_backup_dataset.convert_timeseries_to_annual_trends(
            year_type=year_type
        ),
    ...
    trend_combined, _ = catalogue_calibrated_series.average_timeseries_in_catalogue(
        remove_trend=False, out_data_group=data_group
    )
    return (
        trend_combined,
        data_catalogue_trends_homogenized,
        catalogue_calibrated_series,
    )
