...
def meters_to_gigatonnes(
    ...
    """
    return [
        i * density_of_ice * (area_km2 / 1e6) for i in variables
    ]  # 1e6 to convert area from km2 to m2


def gigatonnes_to_meters(
    variables: Iterable,
    area_km2: float,
    density_of_ice: float = constants.DENSITY_OF_ICE_KG_PER_M3,
) -> list:
...
def meters_to_meters_water_equivalent(
    variables: Iterable,
...
def meters_water_equivalent_to_meters(
    variables: Iterable,
    density_of_water: float = constants.DENSITY_OF_WATER_KG_PER_M3,
    density_of_ice: float = constants.DENSITY_OF_ICE_KG_PER_M3,
) -> list:
...
def meters_water_equivalent_to_gigatonnes(
    variables: Iterable,
    area_km2: float,
    density_of_water: float = constants.DENSITY_OF_WATER_KG_PER_M3,
) -> list:
    ...
    """
    return [
        i * density_of_water * (area_km2 / 1e6) for i in variables
    ]  # 1e6 to convert area from km2 to m2


def gigatonnes_to_meters_water_equivalent(
    variables: Iterable,
    area_km2: float,
    density_of_water: float = constants.DENSITY_OF_WATER_KG_PER_M3,
) -> list:
...
def gigatonnes_to_sea_level_rise(
    variables: Iterable, ocean_area_km2: float = constants.OCEAN_AREA_IN_KM2
) -> list:
    ...
    """
    return [
        abs(variable / ocean_area_km2) * 1e6 for variable in variables
    ]  # 1e6 to convert area from km2 to m2
