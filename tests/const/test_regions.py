...
def test_get_adjusted_area(rgi):
    ...
    adjusted_area_for_reference_year = region.get_adjusted_area(
        start_date=region.area_change_reference_year,
    ...
    adjusted_area_2_years = region.get_adjusted_area(
        start_date=region.area_change_reference_year,
        end_date=region.area_change_reference_year + 2,
        rgi_area_version=rgi,
    )
    assert (
        adjusted_area_2_years < expected_result
    )  # area should now have gotten smaller
    expected_value = (
        expected_result + 1 * (region.area_change / 100) * expected_result
    )  # calcualate expected value
    ...
    adjusted_area_4_years = region.get_adjusted_area(
        start_date=region.area_change_reference_year,
        end_date=region.area_change_reference_year + 4,
        rgi_area_version=rgi,
    )
    np.testing.assert_almost_equal(
        2 * (adjusted_area_2_years - expected_result),
