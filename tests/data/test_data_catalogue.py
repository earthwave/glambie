...
def test_get_filtered_catalogue_by_data_group(example_catalogue):
    ...
    assert (
        example_catalogue.get_filtered_catalogue(data_group="gravimetry")
        .datasets[0]
        .data_group.name
        == "gravimetry"
    )
...
def test_get_filtered_catalogue_by_region_and_data_group(example_catalogue):
    assert (
        len(
            example_catalogue.get_filtered_catalogue(
                region_name="svalbard", data_group="altimetry"
            )
        )
        == 1
    )
...
def test_data_catalogue_from_file():
    catalogue = DataCatalogue.from_json_file(
        os.path.join("tests", "test_data", "datastore", "meta.json")
    )
...
def test_average_timeseries_in_catalogue_both_same(example_catalogue_small):
    ...
    result_timeseries, _ = example_catalogue_small.average_timeseries_in_catalogue(
        remove_trend=False, add_trend_after_averaging=False
    )
    assert np.array_equal(
        result_timeseries.data.changes, example_catalogue_small.datasets[0].data.changes
    )
...
def test_average_timeseries_in_catalogue_example_with_doubled(example_catalogue_small):
    ...
    example_catalogue_small.datasets[0].data.changes = list(
        np.array(example_catalogue_small.datasets[0].data.changes) * 2
    )
    result_timeseries, _ = example_catalogue_small.average_timeseries_in_catalogue(
        remove_trend=False, add_trend_after_averaging=False
    )
    ...
    assert np.array_equal(
        ...
        np.array(example_catalogue_small.datasets[1].data.changes) * 1.5,
    )
...
def test_average_timeseries_in_catalogue_example_with_trends_removed(
    ...
    # remove trends
    result_timeseries_trend_removed, _ = (
        example_catalogue_small.average_timeseries_in_catalogue(
            remove_trend=True, add_trend_after_averaging=False
        )
    )
    result_timeseries_trend_removed_added_after_averaging, _ = (
        example_catalogue_small.average_timeseries_in_catalogue(
            remove_trend=True, add_trend_after_averaging=True
        )
    )
    # now we expect different results
    assert not np.array_equal(
        result_timeseries.data.changes, result_timeseries_trend_removed.data.changes
    )
    assert not np.array_equal(
        list(result_timeseries_trend_removed_added_after_averaging.data.changes),
...
def test_get_common_period_of_datasets_with_gaps(example_catalogue_small):
    ...
    ds_with_gaps.data = TimeseriesData(
        start_dates=[*data.start_dates[:2], data.start_dates[3]],
    ...
    catalogue = DataCatalogue.from_list(
        [example_catalogue_small.datasets[0], ds_with_gaps]
    )
