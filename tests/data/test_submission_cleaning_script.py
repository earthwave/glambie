...
def test_check_glambie_submission_for_errors_date_error(
    test_inputs_path, example_file_check_dataframe
):
    test_csv_with_date_error = os.path.join(
        test_inputs_path, "submissions", "test_glambie_submission_date_error.csv"
    )
    example_file_check_dataframe["local_filepath"] = test_csv_with_date_error
    example_file_check_dataframe = check_glambie_submission_for_errors(
        test_csv_with_date_error, example_file_check_dataframe
    )

    assert not example_file_check_dataframe.loc[
        example_file_check_dataframe.local_filepath.__eq__(test_csv_with_date_error),
        "date_check_satisfied",
    ].values[0]
    assert example_file_check_dataframe.loc[
        example_file_check_dataframe.local_filepath.__eq__(test_csv_with_date_error),
        "nodata_check_satisfied",
    ].values[0]


def test_check_glambie_submission_for_errors_nodata_error(
    test_inputs_path, example_file_check_dataframe
):
    ...
    example_file_check_dataframe = check_glambie_submission_for_errors(
        test_csv_with_nodata_error, example_file_check_dataframe
    )
    assert example_file_check_dataframe.loc[
        example_file_check_dataframe.local_filepath.__eq__(test_csv_with_nodata_error),
        "date_check_satisfied",
    ].values[0]
    assert not example_file_check_dataframe.loc[
        example_file_check_dataframe.local_filepath.__eq__(test_csv_with_nodata_error),
