...
from glambie.processing.process_global_results import (
    _combine_regional_results_into_global,
)
...
def test_combine_regional_results_into_global_in_mwe(example_catalogue):
    combined_result = _combine_regional_results_into_global(
        regional_results_catalogue=example_catalogue, rgi_area_version=7
    )
    dataset1 = example_catalogue.datasets[0]
    dataset2 = example_catalogue.datasets[1]
    adjusted_areas_1 = [
        dataset1.region.get_adjusted_area(start_dates, end_dates, rgi_area_version=7)
        for start_dates, end_dates in zip(
            dataset1.data.start_dates, dataset1.data.end_dates
        )
    ]
    adjusted_areas_2 = [
        dataset2.region.get_adjusted_area(start_dates, end_dates, rgi_area_version=7)
        for start_dates, end_dates in zip(
            dataset2.data.start_dates, dataset2.data.end_dates
        )
    ]
    expected_result = (
        dataset1.data.changes * adjusted_areas_1
...
def test_combine_regional_results_into_global_in_mwe_errors(example_catalogue):
    ...
    dataset1 = example_catalogue.datasets[0]
    dataset2 = example_catalogue.datasets[1]
    adjusted_areas_1 = np.array(
        [
            dataset1.region.get_adjusted_area(
                start_dates, end_dates, rgi_area_version=7
            )
            for start_dates, end_dates in zip(
                dataset1.data.start_dates, dataset1.data.end_dates
            )
    ...
    adjusted_areas_2 = np.array(
        [
            dataset2.region.get_adjusted_area(
                start_dates, end_dates, rgi_area_version=7
            )
            for start_dates, end_dates in zip(
                dataset2.data.start_dates, dataset2.data.end_dates
    ...
    expected_errors = np.sqrt(
        (dataset1.data.errors**2 * adjusted_areas_1**2)
        + (dataset2.data.errors**2 * adjusted_areas_2**2)
    ) / (adjusted_areas_1 + adjusted_areas_2)
...
def test_run_global_results_saves_csv2(tmp_path, example_catalogue):
    ...
    with patch(
        "glambie.processing.process_global_results._homogenize_regional_results_to_calendar_year"
        ...
        patch_homogenize_regional_results_to_calendar_year.return_value = (
            example_catalogue
        )
        ...
        run_global_results(
            ...
            original_data_catalogue=example_catalogue,
