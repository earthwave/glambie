...
def example_catalogue_1():
    return DataCatalogue.from_dict(
        {
...
def example_catalogue_filled():
    data1 = TimeseriesData(
        ...
        changes=np.array([2.0, 5.0, 7.0]),
        errors=np.array([1.0, 1.2, 1.3]),
...
def test_filter_catalogue_with_config_settings_demdiff_and_glaciological(
    ...
    assert all(d.user_group != "lions" for d in datasets_annual.datasets)
    assert all(
        d.data_group != GLAMBIE_DATA_GROUPS["demdiff"] for d in datasets_annual.datasets
    )
    ...
    assert all(d.user_group != "sharks" for d in datasets_trend.datasets)
    assert all(
        d.data_group != GLAMBIE_DATA_GROUPS["glaciological"]
        for d in datasets_trend.datasets
    )
...
def test_filter_catalogue_with_config_settings_with_added_combined_dataset_to_annual(
    ...
    region_config.region_run_settings[data_group.name][
    ...
    assert not all(
        d.data_group == GLAMBIE_DATA_GROUPS["glaciological"]
        for d in datasets_annual.datasets
    )
...
def test_slice_timeseries_at_gaps():
    ts = pd.DataFrame(
        {
            "start_dates": [2012, 2013, 2015, 2016, 2020],
    ...
    expected_split_ts_result = [
        pd.DataFrame(
            {"start_dates": [2012, 2013], "end_dates": [2013, 2014], "changes": [2, 3]}
        ),
        pd.DataFrame(
            {"start_dates": [2015, 2016], "end_dates": [2016, 2017], "changes": [4, 5]}
        ),
        pd.DataFrame({"start_dates": [2020], "end_dates": [2023], "changes": [6]}),
    ]
...
def test_check_and_handle_gaps_in_timeseries(example_catalogue_filled):
    ...
    # introduce a gap
    example_catalogue_filled.datasets[1].data.start_dates[1] = (
        example_catalogue_filled.datasets[1].data.start_dates[1] + 0.5
    )
    assert not example_catalogue_filled.datasets[1].data.is_cumulative_valid()
    result_catalogue, split_dataset_names = check_and_handle_gaps_in_timeseries(
        example_catalogue_filled
    )
    ...
    assert np.array_equal(
        ...
        [
            [
                example_catalogue_filled.datasets[1].user_group + "_1",
...
def test_recombine_split_timeseries(example_catalogue_filled):
    ...
    split_catalogue, split_dataset_names = check_and_handle_gaps_in_timeseries(
        example_catalogue_filled
    )
    recombined_catalogue = recombine_split_timeseries_in_catalogue(
        data_catalogue=split_catalogue,
        names_of_split_datasets_in_catalogue=split_dataset_names,
    )

    assert len(recombined_catalogue) == len(example_catalogue_filled)
    assert (
        recombined_catalogue.datasets[0].user_group
        == example_catalogue_filled.datasets[1].user_group
    )
    assert np.array_equal(
        recombined_catalogue.datasets[0].data.start_dates,
        example_catalogue_filled.datasets[1].data.start_dates,
    )
    assert np.array_equal(
        recombined_catalogue.datasets[0].data.end_dates,
...
def test_extend_annual_timeseries_if_shorter_than_time_window(example_catalogue_filled):
    ...
    timeseries_for_extension.data.start_dates = np.append(
        timeseries_for_extension.data.start_dates, [2013, 2014]
    )
    timeseries_for_extension.data.end_dates = np.append(
        timeseries_for_extension.data.end_dates, [2014, 2015]
    )
    timeseries_for_extension.data.changes = np.append(
        timeseries_for_extension.data.changes, [1.0, 2.0]
    )
    timeseries_for_extension.data.errors = np.append(
        timeseries_for_extension.data.errors, [1.0, 2.0]
    )
