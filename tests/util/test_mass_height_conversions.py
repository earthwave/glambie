...
def test_meters_to_meters_water_equivalent():
    ...
    assert meters_to_meters_water_equivalent(
        meters_list, density_of_ice=density_of_ice, density_of_water=density_of_water
    ) == [
        (20 / density_of_water) * density_of_ice,
        (30 / density_of_water) * density_of_ice,
    ]
...
def test_meters_water_equivalent_to_meters():
    ...
    density_of_ice = 852
    density_of_water = 998
    assert meters_water_equivalent_to_meters(
        mwe_list, density_of_ice=density_of_ice, density_of_water=density_of_water
    ) == [
        (20 * density_of_water) / density_of_ice,
        (30 * density_of_water) / density_of_ice,
    ]
...
def test_gigatonnes_to_meters_water_equivalent():
    ...
    assert gigatonnes_to_meters_water_equivalent(
        gigatonnes_list, 1000, density_of_water=density_of_water
    ) == [
        (1e6 * 50) / (1000 * density_of_water),
        (1e6 * 60) / (1000 * density_of_water),
    ]
...
def test_gigatonnes_to_sea_level_rise():
    gigatonnes_list = [50, 60]
    area_ocean = 3.125e8
    assert gigatonnes_to_sea_level_rise(gigatonnes_list, ocean_area_km2=area_ocean) == [
        abs(50 / area_ocean) * 1e6,
        abs(60 / area_ocean) * 1e6,
    ]
    # now test with default parameter
    assert gigatonnes_to_sea_level_rise(gigatonnes_list) == [
        abs(50 / constants.OCEAN_AREA_IN_KM2) * 1e6,
        abs(60 / constants.OCEAN_AREA_IN_KM2) * 1e6,
    ]
