...
def test_calibrate_timeseries_with_trends(example_trends, example_calibration_timeseries):
    calibrated_ts, distance_matrix = calibrate_timeseries_with_trends(example_trends, example_calibration_timeseries)
    ...
    assert np.array_equal(calibrated_ts[0], np.array(example_calibration_timeseries["changes"]))
    ...
    assert np.array_equal(calibrated_ts[1], np.array(example_calibration_timeseries["changes"]) + expected_correction)
...
def test_combine_calibrated_timeseries(example_calibrated_series, example_distance_matrix):
    p_value = 2
...
def test_combine_calibrated_timeseries_warning_message(example_trends, example_calibration_timeseries):

    example_trends["end_dates"] = [2002, 2006]  # going until 2006 should spark a warning
